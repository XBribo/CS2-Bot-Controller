...
extern "C" BC_EXPORT int BotController_CanSendVoice() { return BotController::VoiceSender::IsAvailable() ? 1 : 0; }

// Return 0 when voice sending is ready, otherwise a negative setup code.
extern "C" BC_EXPORT int BotController_GetVoiceStatus() { return BotController::VoiceSender::GetStatus(); }
...
extern "C" BC_EXPORT int BotController_SendVoiceFrame(int recipientSlot,
                                                      ...
                                                      int audibleMask)
{
    return BotController::VoiceSender::SendVoiceFrame(recipientSlot, senderClient, senderXuid, audio, audioBytes, sampleRate, voiceLevel,
...
{
    if (slot < 0 || slot >= BotController::BuyControllerState::kMaxSlots) return -2;
...
extern "C" BC_EXPORT int BotController_GetBuyPlanItemCount(int slot) { return BotController::BuyControllerState::ItemCount(slot); }

// ---- Motion recording & replay ----

// Begin/stop recording a human slot's per-tick movement. 0 ok / -1 fail.
extern "C" BC_EXPORT int BotController_StartRecord(int slot) { return BotController::MotionRecorder::StartRecord(slot) ? 0 : -1; }

extern "C" BC_EXPORT int BotController_StopRecord(int slot) { return BotController::MotionRecorder::StopRecord(slot) ? 0 : -1; }

// Recorded tick / subtick counts for a slot. <0 on bad slot.
extern "C" BC_EXPORT int BotController_GetRecordedTickCount(int slot) { return BotController::MotionRecorder::RecordedTickCount(slot); }
...
extern "C" BC_EXPORT int BotController_LoadReplay(
    int slot, const BotController::ReplayTick* ticks, int tickCount, const BotController::SubtickMove* subs, int subCount) noexcept
{
    return BotController::MotionRecorder::LoadReplay(slot, ticks, tickCount, subs, subCount) ? 0 : -1;
}
...
extern "C" BC_EXPORT int BotController_LoadReplayExtended(int slot,
                                                          const BotController::ReplayTick* ticks,
                                                          int tickCount,
                                                          const BotController::SubtickMove* subs,
                                                          int subCount,
                                                          const BotController::ReplayCommandFrameData* commands,
                                                          int commandCount,
                                                          const BotController::ReplayMovementExtra* movementExtras,
                                                          int movementExtraCount) noexcept
{
    return BotController::MotionRecorder::LoadReplayExtended(slot, ticks, tickCount, subs, subCount, commands, commandCount, movementExtras,
                                                             movementExtraCount)
               ? 0
               : -1;
}
...
extern "C" BC_EXPORT int BotController_TransferRecordingToReplay(int srcSlot, int dstSlot)
{
    ...
    int gotS = ns > 0 ? BotController::MotionRecorder::CopySubticks(srcSlot, subs.data(), ns) : 0;
    if (gotT <= 0) return -1;
    return BotController::MotionRecorder::LoadReplayExtended(dstSlot, ticks.data(), gotT, subs.data(), gotS, nullptr, 0, nullptr, 0) ? 0
                                                                                                                                     : -1;
}
...
{
    return BotController::InputInjector::SetReplayPawn(slot, reinterpret_cast<void*>(static_cast<uintptr_t>(pawnPtr))) ? 0 : -1;
}

extern "C" BC_EXPORT int BotController_StopReplay(int slot) { return BotController::MotionRecorder::StopReplay(slot) ? 0 : -1; }

// Current replay tick index, or <0 if the slot is not replaying.
extern "C" BC_EXPORT int BotController_GetReplayCursor(int slot) { return BotController::MotionRecorder::ReplayCursor(slot); }

// Total ticks loaded in a slot's replay buffer.
extern "C" BC_EXPORT int BotController_GetReplayTotal(int slot) { return BotController::MotionRecorder::ReplayTotal(slot); }
...
extern "C" BC_EXPORT int BotController_GetBotActiveWeaponDef(int slot) { return BotController::MotionRecorder::BotActiveWeaponDef(slot); }

extern "C" BC_EXPORT uint64_t BotController_GetHookCallCount() { return BotController::InputInjector::HookCallCount(); }

extern "C" BC_EXPORT int BotController_GetLastResolvedSlot() { return BotController::InputInjector::LastResolvedSlot(); }

extern "C" BC_EXPORT uint64_t BotController_GetFinishMoveCallCount() { return BotController::InputInjector::FinishMoveCallCount(); }
...
extern "C" BC_EXPORT int BotController_GetLastPhysicsSlot() { return BotController::InputInjector::LastPhysicsSlot(); }

extern "C" BC_EXPORT uint64_t BotController_GetReplayCommitCount() { return BotController::InputInjector::ReplayCommitCount(); }

extern "C" BC_EXPORT uint64_t BotController_GetSlotResolveCallCount() { return BotController::InputInjector::SlotResolveCallCount(); }

extern "C" BC_EXPORT uint64_t BotController_GetSlotResolveFailureCount() { return BotController::InputInjector::SlotResolveFailureCount(); }
...
extern "C" BC_EXPORT uint64_t BotController_GetLastPawn() { return static_cast<uint64_t>(BotController::InputInjector::LastPawn()); }

extern "C" BC_EXPORT uint32_t BotController_GetLastControllerHandle() { return BotController::InputInjector::LastControllerHandle(); }
...
extern "C" BC_EXPORT int BotController_GetLastControllerIndex() { return BotController::InputInjector::LastControllerIndex(); }
...
extern "C" BC_EXPORT int BotController_GetLastOwnerSlot() { return BotController::InputInjector::LastOwnerSlot(); }
