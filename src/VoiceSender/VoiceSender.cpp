...
static INetworkMessageInternal* FindVoiceMessage()
{
    ...
    if (!g_voiceMessage) g_voiceMessage = g_networkMessages->FindNetworkMessage("CSVCMsg_VoiceData");
...
int GetStatus()
{
    ...
    if (!g_networkMessages) return -2;
    if (!FindVoiceMessage()) return -3;
    return 0;
}

// Return whether all voice sender dependencies are available
bool IsAvailable() { return GetStatus() == 0; }

// Allocate and send one CSVCMsg_VoiceData frame to a recipient
int SendVoiceFrame(int recipientSlot,
                   int senderClient,
                   uint64_t senderXuid,
                   const uint8_t* audio,
                   int audioBytes,
                   ...
                   int audibleMask)
{
    if (!g_engine || !g_networkMessages) return -1;
    if (recipientSlot < 0 || recipientSlot >= 64 || senderClient < 0 || !audio || audioBytes <= 0) return -2;
    if (packetOffsetCount < 0 || packetOffsetCount > 64 || (packetOffsetCount > 0 && !packetOffsets)) return -2;
    ...
    if (sampleRate > 0) voice->set_sample_rate(static_cast<uint32_t>(sampleRate));
    if (std::isfinite(voiceLevel)) voice->set_voice_level(voiceLevel);
    if (sequenceBytes >= 0) voice->set_sequence_bytes(sequenceBytes);
    if (sectionNumber >= 0) voice->set_section_number(static_cast<uint32_t>(sectionNumber));
    if (uncompressedSampleOffset >= 0) voice->set_uncompressed_sample_offset(static_cast<uint32_t>(uncompressedSampleOffset));
    if (numPackets > 0) voice->set_num_packets(numPackets);
    ...
    if (senderXuid != 0) msg->set_xuid(senderXuid);
    if (audibleMask >= 0) msg->set_audible_mask(audibleMask);
    if (tick >= 0) msg->set_tick(static_cast<uint32_t>(tick));
