    ...
    namespace VoiceSender
    {
            ...
            int audibleMask)
        {
            if (!g_engine || !g_networkMessages)
                return -1;
            if (recipientSlot < 0 || recipientSlot >= 64 || senderClient < 0 ||
                !audio || audioBytes <= 0)
                return -2;
            if (packetOffsetCount < 0 || packetOffsetCount > 64 ||
                (packetOffsetCount > 0 && !packetOffsets))
                return -2;
            ...
            if (uncompressedSampleOffset >= 0)
                voice->set_uncompressed_sample_offset(
                    static_cast<uint32_t>(uncompressedSampleOffset));
            if (numPackets > 0)
